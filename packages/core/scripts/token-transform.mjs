...
const LINK_VISITED_VAR = '--hc-color-link-visited';

/** `:root, [data-color="teal"]` + `a:visited` -> one scoped rule per part. */
function scopeSelector(blockSelector, suffix) {
...
/**
 * Bare-anchor rules for `@layer hc.base`, one pair per themed block.
 *
 * `a:visited` carries the baked literal; `a:hover` re-pins the hover colour
 * at the *same* specificity so it still wins on a visited link. Without the
 * hover half, the compound `[data-theme="dark"][data-color="teal"]` blocks
 * (0,2,1) would outrank the plain `a:hover` (0,1,1) in hc.base.css and a
 * hovered visited link would stay stuck on its visited colour.
 */
function emitLinkRules(linkRules) {
  return linkRules.map(({ selector, value }) =>
    `${scopeSelector(selector, 'a:visited')} {\n  color: ${value};\n}\n` +
    `${scopeSelector(selector, 'a:hover')} {\n  color: var(--hc-color-link-hover);\n}\n`
  );
}
...
export function buildTokensCss({ sources, trees }) {
  ...
  for (const src of sources) {
    ...
    // var() there on purpose — resolving it would let a page read the
    // history bit back out through the cascade — so the declaration has to
    // carry a literal colour. A consumer therefore cannot express a themed
    // visited link at all; the token build can, because it already resolves
    // every leaf per theme at build time. Take the literal straight off the
    // declaration this block just emitted, so the rule and the custom
    // property can never drift apart.
    const visited = lines.find((l) => l.startsWith(`${LINK_VISITED_VAR}:`));
    if (visited) {
      linkRules.push({
        selector: src.selector,
        value: visited.slice(LINK_VISITED_VAR.length + 1).trim().replace(/;$/, ''),
      });
    }
  }
  ...
  const indent = (b) => b.replace(/^/gm, '  ').replace(/^ {2}$/gm, '');
  const linkBlocks = emitLinkRules(linkRules);
  const css =
    banner +
    '@layer hc.tokens {\n' +
    blocks.map(indent).join('\n') +
    '}\n' +
    // Bare anchors belong to hc.base, not hc.tokens — hc.base is the later
    // layer, so a rule emitted here still loses to anything the app writes
    // outside the hc layers, and hc.base.css (bundled after this file)
    // supplies the unvisited `a` colour these rules build on.
    (linkBlocks.length
      ? '\n@layer hc.base {\n' + linkBlocks.map(indent).join('\n') + '}\n'
      : '');
...
export const DEFAULT_SOURCES = [
  ...
  // Dark counterparts of the non-default accents, carrying the link colours
  // only. Links are the one accent value that cannot be theme-independent —
  // no single ramp step clears 4.5:1 against both the light and the dark
  // surface — so `[data-color]` alone cannot express them and these compound
  // blocks pick up where the light ones leave off. The default (blue) accent
  // needs no file: `theme.dark` already carries its dark link steps, and the
  // `color.default` block deliberately omits link tokens so it cannot
  // out-order that override. Same two-form selector as the neutral dark
  // blocks, for the same reason (data-theme on an ancestor vs the same
  // element).
  { namespace: 'color.teal.dark',    file: 'color.teal.dark.tokens.json',    selector: '[data-theme="dark"] [data-color="teal"], [data-theme="dark"][data-color="teal"]',       overlay: ['theme.dark', 'color.teal.dark'] },
  { namespace: 'color.lime.dark',    file: 'color.lime.dark.tokens.json',    selector: '[data-theme="dark"] [data-color="lime"], [data-theme="dark"][data-color="lime"]',       overlay: ['theme.dark', 'color.lime.dark'] },
  { namespace: 'color.orange.dark',  file: 'color.orange.dark.tokens.json',  selector: '[data-theme="dark"] [data-color="orange"], [data-theme="dark"][data-color="orange"]',   overlay: ['theme.dark', 'color.orange.dark'] },
  { namespace: 'color.fuchsia.dark', file: 'color.fuchsia.dark.tokens.json', selector: '[data-theme="dark"] [data-color="fuchsia"], [data-theme="dark"][data-color="fuchsia"]', overlay: ['theme.dark', 'color.fuchsia.dark'] },
...
// Accents whose axis file must also carry a dark block (see above). Kept
// separate from AXIS_NAMESPACES so `hc.tokens.color-teal.css` bundles the
// light and dark halves of one accent into a single file, the way the
// neutral ramps do.
export const COLOR_RAMPS_WITH_DARK = ['teal', 'lime', 'orange', 'fuchsia'];

...
Status: **shipped, incl. the §13 pentagon addendum** — PRs 2, 4 and 5 implemented; PR 3 dropped as a
...
## 13. Post-ship addendum — the accent pentagon (2026-08-08)

With no adopters yet, the default *choice of hues* was reviewed on top
of the ladder work. Measured findings, then the change:

1. `semantic.color.warning` resolved to `amber.600` — **bronze**. Its
   consumers are all borders / fills / checked surfaces (3:1 bar, not
   4.5:1), so it now resolves to `amber.500` (3.73:1 on white). A
   bright `#f59e0b`-style amber is 2.1:1 on white — physically
   incompatible with white glyphs; this is as bright as an
   accessible-fill amber gets.
2. The old axis hues clustered: `rose` 9.7° from the error red (a
   rose-themed app drew primary actions and errors in the same color)
   and `indigo` 12.6° from the default blue. Meanwhile teal/cyan,
   orange, and magenta were absent entirely.
3. The fix is the **accent pentagon**: axes at 72° spacing anchored at
   blue — `default` 264.4° / `fuchsia` 336.4° / `orange` 48.4° /
   `lime` 120.4° / `teal` 192.4°. Every vertex ≥ 21° from every status
   hue. A hexagon was evaluated and rejected: anchored at blue, one
   vertex lands **2.9°** from the error red — the rose defect, worse.
   The answer to "I want a sixth accent" is the theme builder, which
   runs any hue through the same ladder.
4. Four new primitive ramps back the axes (`fuchsia`/`orange`/`lime`/
   `teal`, generated by `build-ramp.mjs`); the seven original ramps are
   untouched and stay as primitives (status colors, chart series, and
   syntax tokens reference them). `data-color="indigo|emerald|rose|
   amber"` is **removed**, flagged in the CHANGELOG under the
   no-adopters window.
5. `test/ramp.test.mjs` now asserts the pentagon spacing and the
   ≥ 20° status-hue clearance, so a future axis cannot quietly drift
   into a collision. Contrast floor across the five axes: teal at
   4.83:1 (was: emerald 4.76:1).

Caveat recorded for honesty: at ladder lightness, `lime.600`
(`#657907`) reads olive rather than lime-bright, and `orange.600`
(`#ae4f07`) is a burnt orange — the same L 0.54 physics that made
amber bronze applies to every warm hue. That is the trade the pentagon
makes for white text and uniform steps; the light steps (300–400) of
those ramps carry the vivid versions.

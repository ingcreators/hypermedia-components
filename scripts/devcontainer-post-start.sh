...
if ! git config --global --get-all safe.directory 2>/dev/null | grep -Fxq "/workspace/hypermedia-components"; then
  git config --global --add safe.directory /workspace/hypermedia-components
fi

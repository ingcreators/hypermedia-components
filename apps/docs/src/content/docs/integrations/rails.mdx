...
<body>
  ...
  <%# No CSRF script needed: csrf_meta_tags renders
      meta[name="csrf-token"], and the auto-init behaviors bundle's
      installCsrfHeader() adds it to every htmx request — header name
      X-CSRF-Token, exactly what Rails expects. %>
</body>
</html>
```
...
- `csrf_meta_tags` in the layout plus the auto-init
  `installCsrfHeader()` (see the layout snippet above) covers every
  htmx verb — no hand-written listener needed.

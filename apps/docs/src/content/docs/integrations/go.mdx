...
[`gorilla/csrf`](https://github.com/gorilla/csrf)) and render the
kit's blessed meta tag in the layout — the auto-init
`installCsrfHeader()` behavior forwards it on every htmx request, no
script needed:
...
(`gorilla/csrf` exposes `csrf.Token(r)`). See
[htmx → CSRF tokens](/hypermedia-components/integrations/htmx/#csrf-tokens--the-blessed-convention)
for the convention, including how to layer extra headers with your own
`htmx:configRequest` listener if you need them.

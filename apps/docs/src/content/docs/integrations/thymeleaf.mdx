...
<body>
  ...
  <script>
    ...
    // Spring exposes the header NAME dynamically via _csrf_header, so
    // this is the one stack where the kit's static
    // meta[name="csrf-token"] + installCsrfHeader() convention doesn't
    // fit — a small listener reads both metas instead.

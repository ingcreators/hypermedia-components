...
<head>
    ...
    @* Antiforgery token for htmx requests — read by the auto-init
       installCsrfHeader() behavior; data-header renames the header
       to what ASP.NET Core expects. *@
...
`_Layout.cshtml` snippet above renders the token into the kit's
blessed `<meta name="csrf-token">` tag with
`data-header="RequestVerificationToken"`; `installCsrfHeader()` (in
the auto-init behaviors bundle) adds it to every htmx request — no
hand-written `htmx:configRequest` hook.

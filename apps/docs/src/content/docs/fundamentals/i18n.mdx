...
| `count.of` | `{used} / {max}` | the [character count](/hypermedia-components/components/input/#character-count) next to a bounded field (`{used}` / `{max}` / `{remaining}` params) |
| `count.used` | `{used}` | the character count when the field has no limit |
| `dirtyguard.leave` | `You have unsaved changes. Leave this page?` | the boosted-navigation confirm from [unsaved-changes](/hypermedia-components/recipes/unsaved-changes/) (tab-close prompts stay browser-generic) |
| `sortable.handle` | `Reorder` | glyph-only sortable handle `aria-label` (fallback) |
| `sortable.grabbed` | `Grabbed. Position {index} of {count}.` | keyboard grab announced via `role="status"` |
| `sortable.moved` | `Position {index} of {count}` | keyboard move announcement |
| `sortable.dropped` | `Dropped. Position {index} of {count}.` | committed reorder announcement |
| `sortable.cancel` | `Reorder cancelled` | cancelled reorder announcement |
...
| `networkRetry.failed` | `The request didn't reach the server — check your connection and retry.` | the no-answer alert from [network-retry](/hypermedia-components/recipes/network-retry/) |
| `networkRetry.retry` | `Retry` | its Retry button label |
...
- `hasMessage(key)` — whether the catalog can resolve a key (built-in
  default or override), as opposed to falling back to the key itself.
...
Servers render UTC — the only honest thing a server clock can say —
and the auto-installed `installTime()` rewrites the visible text in
the viewer's zone and language via `Intl`:
...
- `data-hc-time="relative"` — "3 minutes ago" (`Intl.RelativeTimeFormat`,
  refreshed every 30 s); the absolute localized timestamp goes into
  `title` unless one exists.
- `data-hc-time="datetime" | "date" | "time"` — absolute local
  rendering; `data-hc-time-style="short | medium | long | full"` picks
  the style.
- The `datetime` attribute is never touched (the wire truth), the
  server-rendered text is the no-JS fallback, unparseable values are
  left alone, and the language follows the closest `[lang]`. Content
  swapped in by htmx is localized automatically.

No message keys are involved — `Intl` carries the language.

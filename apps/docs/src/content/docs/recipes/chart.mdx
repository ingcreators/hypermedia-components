...
Plot renders wherever a DOM exists. Give it linkedom's `document` on
the server and ship the SVG inside the response — the page then needs
no client-side Plot at all. Emit the figure with
`data-state="rendered"` (plus the `hc-sr-only` table and the
`aria-hidden` SVG): `installChart()` recognizes already-rendered
figures and leaves them alone, so SSR'd and client-rendered charts
coexist. Tokens are client-side CSS — pass explicit colors when
SSR'ing. The

...
| Failure, show an error in the slot | `200` + the alert fragment (as above); keeping a non-2xx status + `HX-Reswap: innerHTML` also works, but needs an `htmx:beforeSwap` allowance (or `htmx.config.responseHandling`) for that status — htmx ≥ 2 does not swap non-2xx by default |

...
breakdown waits behind the **Reasons (N)** rail on the panel **docked
beside the grid**, which you can drag or resize from the keyboard.
Both stay live, which is the point: click a reason and the rows behind
it are still there.
...
hundred failures it is the only shape that works. (In the live demo
above the link is illustrative — a real app points it at the grid URL
with the failure filter applied.)

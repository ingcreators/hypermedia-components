...
affordance. The keyboard counterpart: **Escape dismisses the toast
that contains the focus**, so a sticky toast (`duration: 0`) should
carry an `action` — its button is what makes the toast focusable in
the first place. Vertical drags still scroll

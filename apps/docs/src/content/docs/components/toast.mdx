...
affordance (keyboard users rely on auto-dismiss). Because of that, a
**sticky toast (`duration: 0`) must carry an `action`** — its button is
the only keyboard-reachable way to dismiss it. Vertical drags still scroll

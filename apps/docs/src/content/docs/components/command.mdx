...
```html
<dialog class="hc-command-dialog" data-hc-command-hotkey="k">
  <div class="hc-command">
    <input class="hc-command__input" type="text" role="combobox" autofocus
           aria-label="Command menu" placeholder="Type a command…">
    <div class="hc-command__list" role="listbox">
...
Put `data-hc-command-hotkey="k"` on the `<dialog>` (any single key;
...
```html
<dialog class="hc-command-dialog" data-hc-command-hotkey="k">…</dialog>
```
...
| Key | Action |
| --- | --- |
| Type | Filter items; empty groups and their headings hide; an empty state shows when nothing matches. |

...
htmx の `data-hx-trigger="mouseenter"`(または自分で配線するなら
`hc:hovercardrequest`)と組み合わせて、初回ホバー時にカードの本文を
取得します。
...
アプリ側の小さなラッパーで、カードが初めて開いたときに
`hc:hovercardrequest` を発火できます。(`installHovercard` は現在
これを発火しません — ドキュメント化された拡張パターンとして示して
います。)

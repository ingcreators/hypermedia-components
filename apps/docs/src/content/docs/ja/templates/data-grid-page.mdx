...
- **並び替える**（ヘッダーをクリック）。同値は主キーで割るので、ページング
  が重複・欠落しません。
- **レコードを開く**（同一性の列から）。ここでは*のぞき見*で開きます。
  ドキュメントのページはルートが 1 つだからです。実際のアプリでは
  このクリックは**画面遷移**です（Gmail は画面を置き換え、Fiori は列に
  分割し、Salesforce はページを与えます）。隣にある行の `href` がその
  ページです。どちらでもヘッダーは「出口＝先頭、`n / 24 ‹ ›`＝末尾」で、
  **次へはページ境界を越え**（サーバーがクエリを再実行するため）、保存
  すると背後の行が out-of-band で更新されます。

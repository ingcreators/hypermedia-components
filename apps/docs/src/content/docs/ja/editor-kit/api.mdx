...
**挿入点**は childNodes インデックス*または* `{ before: Element |
null }`(`null` で末尾追加)です — 要素ベースの UI コード
...
`pickBlock(target, { root, manifest })` は、クリックターゲットを
...
const dnd = createDragController({
  root: canvas,                      // 必須
  frame: canvasIframe,               // キャンバスを収める iframe(下記参照)
...
- **document をまたぐドラッグ**: キャンバスが iframe 内にある場合、
  ...
  フレーム境界を越えません。iframe を `frame` に渡すと、コントローラは
  両方の document をリッスンし、ホストビューポート座標をフレームの
  rect を通じてキャンバス座標へ変換します — 親 document のパレットからの
  `startInsert` がそのまま完了します。
